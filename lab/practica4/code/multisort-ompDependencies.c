...
void merge(long n, T left[n], T right[n], T result[n*2], long start, long length, int level) {
...
void multisort(long n, T data[n], T tmp[n], int level) {
    if (n >= MIN_SORT_SIZE*4L) {
        // Recursive decomposition
        if (!omp_in_final()) {
			#pragma omp task final (level >= CUTOFF) depend(out: data[0])
			multisort(n/4L, &data[0], &tmp[0], level+1);
			#pragma omp task final (level >= CUTOFF) depend(out: data[n/4L])
			multisort(n/4L, &data[n/4L], &tmp[n/4L], level+1);
			#pragma omp task final (level >= CUTOFF) depend(out: data[n/2L])
			multisort(n/4L, &data[n/2L], &tmp[n/2L], level+1);
			#pragma omp task final (level >= CUTOFF) depend(out: data[3L*n/4L])
			multisort(n/4L, &data[3L*n/4L], &tmp[3L*n/4L], level+1);

			#pragma omp task final (level >= CUTOFF) depend(in: data[0], data[n/4L]) depend(out: tmp[0])
			merge(n/4L, &data[0], &data[n/4L], &tmp[0], 0, n/2L, level+1);
			#pragma omp task final (level >= CUTOFF) depend(in: data[n/2L], data[3L*n/4L]) depend(out: tmp[n/2L])
			merge(n/4L, &data[n/2L], &data[3L*n/4L], &tmp[n/2L], 0, n/2L, level+1);

			#pragma omp task final (level >= CUTOFF) depend(in: tmp[0], tmp[n/2L])
